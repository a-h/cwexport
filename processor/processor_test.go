...
func TestGetIntervalCount(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:              "When timeRange is less than 1 minute, there are no intervals",
			startTime:         time.Date(2022, time.January, 1, 9, 0, 0, 0, time.UTC),
			endTime:           time.Date(2022, time.January, 1, 9, 0, 5, 0, time.UTC),
			expectedIntervals: 0,
		},
		{
			desc:              "When startTime is 10 minutes before endTime, then there are 10 intervals",
			startTime:         time.Date(2022, time.January, 1, 9, 0, 0, 0, time.UTC),
...
func TestProcess(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			startTime:       time.Date(2022, time.January, 1, 9, 0, 0, 0, time.UTC),
			samples:         []cw.Sample{},
			expectedEndtime: time.Date(2022, time.January, 1, 12, 0, 0, 0, time.UTC),
			expectedSamples: 0,
